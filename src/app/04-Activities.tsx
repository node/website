...
export const ActivitySection = () => {
    ...
    const currentCategoryConfig = _.find(categories, { id: currentCategory })
    if (!!currentCategoryConfig) {
        currentCategoryName = currentCategoryConfig.name;
    }
    return <section
    ...
    >
        ...
        <div className='py-1 md:py-2 bg-coolGray-900'>
            <div className='container px-4 mx-auto'>
                <div className='md:max-w-5xl mx-auto mb-8 md:mb-16 text-center'>
                    ...
                    <h3 className='mb-4 text-3xl md:text-5xl leading-tight text-white font-bold tracking-tighter' id="activities">
                        近期动态
                    </h3>
                    <p className='mb-10 text-lg md:text-xl text-coolGray-400 font-medium'>
              ...
              />
                ...
                <ul className='flex flex-wrap mb-8 -mx-2 text-center'>
                    <li className='w-full md:w-auto px-2'>
                        <a
                            className='inline-block w-full py-2 px-4 mb-4 md:mb-0 text-sm text-coolGray-600 hover:text-blue-900 hover:bg-blue-200 font-bold rounded-md hover:shadow-sm'
                            href='/#activities'
                        >
                            全部分类
                        </a>
                    ...
                    {categories.map((category) => {
                        let defaultClass = `text-coolGray-600`;
                        if (category.id === currentCategory) {
                            defaultClass = `text-blue-900 bg-blue-200`
                        }
                        return <li className='w-full md:w-auto px-2' key={category.id}>
                            <Link
                                className={`inline-block w-full py-2 px-4 mb-4 md:mb-0 text-sm ${defaultClass} hover:text-blue-900 hover:bg-blue-200 font-bold rounded-md hover:shadow-sm`}
                                href={`?category=${category.id}#activities`}
                                scroll={true}
                            >
                                {category.name}
                            </Link>
                ...
                <div className='flex flex-wrap -mx-4 mb-12 md:mb-20'>
                    {activities.map((activity) => {
                        ...
                        return <div className='w-full md:w-1/2 px-4 mb-8' key={activity.title}>
                            ...
                            <div className='mb-4'>
                                <a
                                    className='inline-block py-1 px-3 text-xs leading-5 text-blue-900 font-medium uppercase bg-blue-100 hover:bg-blue-200 rounded-full shadow-sm'
                                    href={`?category=${activity.category}#activities`}
                                >
                                    {activity.category}
                                </a>
                            </div>
                            ...
                            <a
                                className='inline-block mb-4 text-2xl leading-tight text-white hover:text-coolGray-200 font-bold hover:underline'
                                href='#'
                            >
                                {activity.title}
                            </a>
